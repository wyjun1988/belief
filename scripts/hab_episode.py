...
# 정적 앵커 = gt0 (미러 규약·COM 좌표가 이미 적용된 것) 에서 — objs(원점·미러 전) 로 만들면 규약이 어긋난다
# (2026-09-03: v3c 에서 static 만 미러가 빠져 거리 정합·yaw 투표가 오염됐다)
_moved = {m["oid"] for m in moves}
static = {oid: dict(type=v["type"], room=v["room"], pos=list(v["pos"])) for oid, v in gt0.items() if oid not in _moved}
