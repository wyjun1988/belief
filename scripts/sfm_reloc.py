...
ap.add_argument("--no-reject-outside", dest="reject_outside", action="store_false", help="평면도 밖 기권을 끈다(진단용)")
ap.add_argument("--reject-outside", action="store_true", default=True, help="정렬 뒤 어느 방 폴리곤에도 들어가지 않는 live 프레임을 기권 처리 — 유령 복제(잘못 등록된 사본)를 GT 없이 거른다. 평면도는 사용자 입력")
