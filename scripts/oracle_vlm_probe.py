...
def ask(paths, q):
    if BACKEND == "mlx":
        prompt = apply_chat_template(processor, cfg, q, num_images=len(paths)); r = generate(model, processor, prompt, paths, max_tokens=200, verbose=False, temperature=0.0)
...
def run_one(hn, oid, typ, moved, rec_path, ctr0, later_paths, meta):
    """선택된 프레임으로 질문 한 번. 변형 A 박스 없음 / B 기록 장면에 GT 중심 박스."""
    a = typ.replace("_", " ").lower()
    paths = [prep(rec_path, 1, ctr0 if VAR == "B" else None)] + [prep(p, i + 2) for i, p in enumerate(later_paths)]
    q = ("Image 1 shows %s at its recorded place in a house%s. Images 2-%d are later views of that same place, in time order. "
         "Look carefully: is the %s still at its recorded place in the later images? Answer with JSON only: "
         "{\"still_there\": \"yes\"|\"no\"|\"unsure\", \"seen_in\": [image numbers (2-%d) where the %s is visible], \"confidence\": 0-100}"
         % (("a " if a[:1] not in "aeiou" else "an ") + a, " (inside the red box)" if VAR == "B" else "", len(paths), a, len(paths), a))
    txt = ask(paths, q); js = parse(txt) or {}; truth = "no" if moved else "yes"; ans = str(js.get("still_there", "")).lower()
    row = dict(house=hn, oid=oid, type=typ, moved=moved, truth=truth, ans=ans, conf=js.get("confidence"), seen_in=js.get("seen_in"), variant=VAR, model=MODEL, raw=txt[:300]); row.update(meta)
...
for hd in sorted(glob.glob(ROOT + "/house_*")):
    ...
    for oid, v0 in g["gt0"].items():
        ...
        row, truth, ans = run_one(hn, oid, v0["type"], oid in mv, rec_path, ctr0, later_paths, meta)
        out.write(json.dumps(row, ensure_ascii=False) + "\n"); out.flush(); n += 1; stat[(truth, ans)] += 1
        if MAX_OBJ and n >= MAX_OBJ: break
    if MAX_OBJ and n >= MAX_OBJ: break
