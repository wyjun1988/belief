...
for hd in sorted(glob.glob("data/thor2z/house_*")):
    ...
    for oid, v in g["gt0"].items():
        ...
        def soft(idx):
            base = float(np.median(TS[ok]))
            acc = {r: 0.0 for r in rids}
            for i in idx:
                w = max(0.0, float(TS[i]) - base)      # 물체별 자기보정 가중
                rs = RS.setdefault(i, rscore(i))
                for r in rids: acc[r] += w * rs[r]
            return max(rids, key=lambda r: acc[r])
        res.setdefault("agent", []).append(Counter(arm[order[:25]]).most_common(1)[0][0] == tg)
        for K in KS:
            res.setdefault(("hard", K), []).append(hard(order[:K]) == tg)
            res.setdefault(("soft", K), []).append(soft(order[:K]) == tg)
        if gtf is not None:
            res.setdefault("oracle_frame", []).append(hard(gtf) == tg)
KS = (5, 10, 25, 50)
lab = {"agent": "에이전트 방 다수결 (지금 방식)", "oracle_frame": "(참고) 프레임만 GT"}
for K in KS:
    lab[("hard", K)] = "앵커조합 하드투표 상위%d" % K
    lab[("soft", K)] = "**앵커조합 점수가중 상위%d**" % K
print("=== 실전 국소화 [%s 질의] · 조건① 타겟 %d개 ===" % (MODE, len(res["agent"])))
for k in ["agent"] + [("hard", K) for K in KS] + [("soft", K) for K in KS] + ["oracle_frame"]:
