...
# ⚠️ 2026-08-20 정정 — **여백·시점여백으로 판정하지 말 것**

아래 문서의 여백/시점여백 표들은 **편향된 지표**로 잰 것이다. 판정은 **순위 지표**로 한다.
...
**중앙값에서 최댓값을 뺀다.** 후보가 늘수록 최댓값이 위로 밀리므로 표현이 좋든 나쁘든
음수로 나온다. 실제로 집 단위로 후보를 줄여도 시점여백이 −0.067 → **−0.059** 로
거의 안 변했다 — 지표가 후보 수에 반응하지 않는다는 뜻이다.

## 우연 기준선을 안 봤다

| 조건 | 후보 수 | **우연** | DINO-VLAD top-1 | **배수** |
|---|---|---|---|---|
| 전수 355곳 | 355 | **0.28%** | 51% | **180배** |
| 집 단위 | 중앙 4 · 평균 12 | 8~22% | 51% | **2.3~6배** |

앞서 이 51% 를 "동전 던지기" 로 읽었는데 **오독이다.** 우연이 0.28% 인 과제에서
51% 는 180배다. **표현은 장소 정보를 상당히 담고 있다.**

## 판정 기준을 바꾼다

| 이전 결론 | **정정** |
|---|---|
| 표현이 장소를 못 담는다 | **담는다** — 우연의 2~180배 |
| 무학습으로는 답이 없다 | **근거 부족** — 편향 지표로 내린 판정이었다 |
| 병목 = 표현 자체 | 병목 = **top-1 51% 로는 시스템을 못 세운다**(다른 문제) |

`place_repr_bench.py` 가 이제 다음을 찍는다(여백 계열은 참고용으로 남김):

- **top-1** 과 **우연**(1/후보수) 과 **배수**
- **recall@3** — 부재 판정은 후보 몇 개를 훑어도 되므로 이쪽이 실용 지표다
- **시점순위** — 같은 장소를 다른 각도에서 본 프레임이 남의 장소보다 위에 오는 비율.
  0.5 미만이면 각도가 바뀔 때 남의 장소에 밀린다는 뜻이다. **이것이 시점여백을
  대체하는 정직한 지표다.**
...
```bash
git pull
python -u scripts/place_repr_bench.py \
  --root data/scenediff/scenediff_benchmark/data --device cuda --group home \
  --nframes 8 --vlad-k 128 --dino facebook/dinov2-large \
  --save repr_home_L.npz --reps clip_cls,dino_cls,dino_vlad,owlvec | tee out_home_rank.log
```

**보는 것**: `배수` 와 `recall@3`, 그리고 `시점순위`. 시점순위가 0.5 를 넘으면
각도 문제는 없는 것이고, 남는 문제는 순위를 더 올리는 것뿐이다.
