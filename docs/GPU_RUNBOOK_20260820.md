...
이전 판(44곳 축소)의 결론은 전수에서 **뒤집혔다.** 이 문서가 기준이다.
...
③은 장소 수에 따라 커지므로 감쇠는 예상되지만, **44 → 355 에서 부호가 바뀔 만큼
가파르다.** 검색 풀이 3,106 → 10,891프레임일 때 hit@5 가 0.75 → 0.30 으로 무너진
것과 같은 현상이다. **실사용 집은 장소가 수십~수백이므로 전수가 현실 조건이다.**
