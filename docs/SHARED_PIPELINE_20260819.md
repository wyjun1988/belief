...
→ **운용 설계**: 이웃 attribute 는 구축 시 사전계산(물체당 1회)하고, 질의 시점
기본 경로는 LLM-free(attribute+원문, 0.55/0.60). 라우팅(vqa/belief)을 위해
어차피 LLM 을 부르는 경우에만 질문별 이웃(0.70)을 같은 호출에 얹는다 —
비용을 더 내는 게 아니라 이미 내는 호출의 부산물로 받는 것이다.
