...
Qwen 이 키워드+이웃(같이 있을 물체·장소)을 JSON 으로 추출한다. 확장 질의로
물체·위치 hit@5 **0.55 → 0.70**.

**이웃의 출처를 실측으로 정했다** — 원안은 "DB 관측에서 주변 물체를 모은다"
였는데, 두 출처를 같은 채점으로 맞대니 반대가 나왔다:

| 이웃 출처 | 전체 hit@5 | 물체·위치 |
|---|---|---|
| 키워드 단독 | 0.34 | 0.30 |
| **Qwen 세계지식** ("보통 어디 근처에 있나") | **0.49** | **0.70** |
| 관측 PMI (동시출현 씬그래프) | 0.38 | 0.50 |
| 둘 결합 | 0.45 | 0.55 |

관측 이웃이 지는 이유: PMI 는 같은 지각(CLIP 존재판정)에서 나오므로 **잡음이
검색과 상관**되고(`mesh bag → plastic bag, knife sheath, red onion`), Qwen 이웃은
"있을 법한 장소"(`island, counter, kitchen`)를 가리켜 배치 사건을 찾는 검색
목적에 맞는다. **씬그래프(관측)의 역할은 검색이 아니라 belief 의 사전분포와
부재 판정의 문맥이다** — 층이 다르다.

**세계지식 이웃의 사전계산 — attribute 로 넣되, 질문 문맥이 없으면 −0.15 다.**
"세계지식은 물체의 속성이니 씬그래프 구축 시 attribute 로 박아두면 질의 시점
LLM(1.6초/질문)이 필요 없다"는 제안을 실측했다:

| 이웃 생성 방식 | 질의 시점 비용 | 전체 | 물체·위치 |
|---|---|---|---|
| 질문별 Qwen (질문 문맥 포함, 현행) | LLM 1.6초 | 0.49 | **0.70** |
| 물체 attribute 사전계산 | **0** | **0.55** | 0.55 |
| attribute + 질문 원문 연결 | **0** | **0.55** | 0.60 |

attribute 단독이 물체·위치에서 지는 이유는 **인스턴스 모호성**이다 — `soap` 의
attribute 는 `bathroom, shower, towel` 인데 질문 문맥은 부엌의 주방세제였다.
질문 문맥이 어느 인스턴스인지 갈라준다. 질문 원문(공짜)을 붙이면 0.60 까지
회복되고 전체 hit@5 는 오히려 최고(0.55)다.

**관측 후보 + LLM 안정성 선별도 실측했다** — "관측된 주변 물체 리스트를 주고,
양파처럼 없어지기 쉬운 것은 빼고 세제처럼 같이 있기 쉬운 것만 고르게 하자"는
제안. 같은 실행에서 비교(수치가 위 표와 약간 다른 것은 키워드 정규화 차이):

| 이웃 | 전체 | 물체·위치 | 질적 관찰 |
|---|---|---|---|
| 질문별 Qwen (상한) | 0.49 | **0.70** | — |
| 관측 PMI top4 (선별 없음) | 0.38 | 0.50 | `salt → lid, glass lid, kitchen tongs` (잡음) |
| 상식 attribute (관측 없음) | 0.51 | 0.50 | `soap → bathroom, shower` (인스턴스 오류) |
| **관측 후보 + Qwen 선별** | 0.41 | **0.55** | `soap → sink, counter, kitchen drawer` — **오류·잡음 둘 다 교정** |

선별은 의도대로 됐다 — 인스턴스 오류가 고쳐지고(soap 이 부엌으로) 일시적
동시출현이 걸러졌다. 그런데 점수는 +0.05 에 그친다. 남은 격차(0.55 vs 0.70)의
원인은 앵커의 품질이 아니라 **질문 문맥**이다 — "설거지하다가" 같은 단서는
질의 시점에만 존재하므로 어떤 사전계산으로도 대체할 수 없다.

→ **운용 설계**: 이웃 attribute 는 구축 시 사전계산(물체당 1회, **관측 후보 +
안정성 선별** 방식 권장 — 검색 점수는 동률이지만 이 집에 접지된 메타데이터라
belief·부재 문맥에도 재사용된다)하고, 질의 시점 기본 경로는 LLM-free
(attribute+원문, ~0.60). 라우팅(vqa/belief)을 위해 어차피 LLM 을 부르는 경우에만
질문별 이웃(0.70)을 같은 호출에 얹는다 — 이미 내는 호출의 부산물이다.
...
| ⑤ | 확장 질의 이웃은 **DB 관측에서** 모은다 | 관측(PMI) 이웃 0.50 vs Qwen 세계지식 이웃 **0.70** — 관측 이웃은 지각 잡음이 검색과 상관돼 손해 | 검색 이웃은 **Qwen 지식**으로. 관측 그래프는 belief·부재 층에서만 쓴다 |
